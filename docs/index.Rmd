...
The above Maxima code chunk was set with option `output.var = "foo"`

````

```{maxima, output.var = "foo"}`r ''`
integrate(1 / (1 + x^4), x);
```

````

which captured the parsed result(s) as R-code that can subsequently be evaluated in R e.g., to evaluate an expression in the context of a data.frame
...
```{maxima, fig.cap = "plot2d()", fig.align="center"}
r: (exp(cos(t))-2*cos(4*t)-sin(t/12)^5)$
plot2d([parametric, r*sin(t), r*cos(t), [t,-8*%pi,8*%pi]]);
```

```{maxima, fig.cap = "plot3d()", fig.align="center"}
...
```{maxima, fig.cap = "draw()", fig.align="center"}
example1:
...
```{maxima, fig.cap = "draw2d()", fig.align="center"}
draw2d(
...
```{maxima, fig.cap = "draw3d()", fig.align="center"}
draw3d(
   dimensions = [1000, 1000],
   surface_hide      = true,
   axis_3d           = false,
