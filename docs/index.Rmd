...
Note: In this document where I'm mixing direct usage and the knitr engine, I'm setting the chunk option `results="markup"` for the direct usage.

In direct use, one also change the output format using `maxima.setformat()`. Possible options are `"linear"` (default), `"latex"`, `"MathML"` and `"text2d"`. The last options is probably convenient for console output
...
Finally, a session can be restarted and stopped, notably for the effect of getting a new clean Maxima session

```{r, results="markup"}
maxima.start()
maxima.start(restart = TRUE)
maxima.setformat("text2d")
maxima.get("integrate(x / (1 + x^3), x);")
maxima.stop()
maxima.stop()
```
